...
export const forgotPassword = catchAsync(async (request, res) => {
	...
	const resetPasswordToken = await tokenService.generateResetPasswordToken(request.body.mobile_number);
...
export const resetPassword = catchAsync(async (request, res) => {
	await authService.resetPassword(request.query.token as string, request.body.password);
